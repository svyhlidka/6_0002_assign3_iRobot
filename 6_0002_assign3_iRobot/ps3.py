...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for i in range(width):
            for j in range(height):
                self.tiles[(i,j)]=random.randint(1,10)
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if not (m,n) in self.clean_tiles:
            self.tiles[(m,n)] -= capacity
            if self.tiles[(m,n)] <= 0: 
                self.tiles[(m,n)] = 0
                self.clean_tiles.append((m,n))
    ...
    def get_num_cleaned_tiles(self):
        ...
        for m in range(0,self.width):
            for n in range(0,self.height):
                if self.tiles[(m,n)] == 0: total_cleaned += 1
    ...
    def is_position_in_room(self, pos):
        ...
        """
        return (math.floor(pos.get_x()) in range(1,self.width)) and (math.floor(pos.get_x()) in range(1,self.heigth))
       # raise NotImplementedError
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.position = Position(random.uniform(1,room.width), \
                         random.uniform(1,room.height))
...
class EmptyRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        Returns: True if pos is in the room, False otherwise.
        """
        return (math.floor(pos.get_x()) in range(0,self.width)) \
                  and (math.floor(pos.get_y()) in range(0,self.height))
        
    def get_random_position(self):
        """
        Returns: a Position object; a valid random position (inside the room).
        """
        p = Position(random.randint(1,self.width), \
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        return (math.floor(pos.get_x()) in range(0,self.width)) \
                     and (math.floor(pos.get_y()) in range(0,self.height)) \
                     and not self.is_position_furnished(pos) 
    ...
    def get_num_tiles(self):
        ...
        for m in range(0,self.width):
            for n in range(0,self.height):
                if not (m,n) in self.furniture_tiles: total_tiles += 1
        return total_tiles
    ...
    def get_random_position(self):
        ...
        """
        i = 0
        p = Position(random.randint(1,self.width),random.randint(1,self.height))
        while i < 100:
            if not self.is_position_furnished(p): return p
            p = Position(random.randint(1,self.width),random.randint(1,self.height))
            i += 1
        raise ValueError('Random Position Error!')    
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
#        print('old:',str(self.position),self.get_robot_direction())
        new_position = self.position.get_new_position(self.direction, self.speed)
#        print('new:',str(new_position),self.get_robot_direction())
        if self.room.is_position_valid(new_position):
            self.position = new_position
        else:
#            print('position error!')
            self.direction = random.uniform(0.0,360.0)
#            print('ia:',str(self.position),self.get_robot_direction())
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    room = room_type( width, height, dirt_amount)
    if is_furnished:
...
#run_simulation(5, 1, 1, 20, 20, 3, 1.0, 1, StandardRobot, EmptyRoom)

#print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 5, 5, 3, 1.0, 50, StandardRobot, EmptyRoom)))
#print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.8, 50, StandardRobot, EmptyRoom)))
#print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.9, 50, StandardRobot, EmptyRoom)))
#print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 20, 20, 3, 0.5, 50, StandardRobot, EmptyRoom)))
#print ('avg time steps: ' + str(run_simulation(3, 1.0, 1, 20, 20, 3, 0.5, 50, StandardRobot, EmptyRoom)))
...
def show_plot_compare_strategies(title, x_label, y_label):
    ...
    for num_robots in num_robot_range:
        ...
        times1.append(run_simulation(num_robots, 1.0, 1, 20, 20, 3, 0.8, 20, StandardRobot, EmptyRoom))
        times2.append(run_simulation(num_robots, 1.0, 1, 20, 20, 3, 0.8, 20, FaultyRobot, EmptyRoom))
...
def show_plot_room_shape(title, x_label, y_label):
    ...
    for width in [10, 20, 25, 50]:
        ...
        times1.append(run_simulation(2, 1.0, 1, width, height, 3, 0.8, 200, StandardRobot, EmptyRoom))
        times2.append(run_simulation(2, 1.0, 1, width, height, 3, 0.8, 200, FaultyRobot, EmptyRoom))
